  ...
  class ReadClass {
    ...
    void 存在しない名前を指定したとき404エラーを返すこと() throws Exception {
      ...
      mockMvc.perform(MockMvcRequestBuilders.get("/users/names?name=unknownUser"))
          ...
          .andExpect(MockMvcResultMatchers.jsonPath("$.message")
              .value("user not found containing name: " + "unknownUser"))
    ...
    void 存在しないIDを指定したとき404エラーを返すこと() throws Exception {
      ...
      mockMvc.perform(MockMvcRequestBuilders.get("/users/0"))
          ...
          .andExpect(MockMvcResultMatchers.jsonPath("$.error").value("Not Found"))
          .andExpect(
              MockMvcResultMatchers.jsonPath("$.message").value("user not found with id: " + 0))
      ...
      String requestBody = """
          {
                    ...
                    "birthday": "2000-01-01"
                    "gender": "female"
                    "phoneNumber": "090-1234-5555"
                    "email": "newUser@test.co.jp"
                    "password": "NewUser@0101"
          }
          """;
  ...
  class UpdateClass {
    ...
    void 存在しないIDでユーザーの更新をしようとすると404エラーを返すこと() throws Exception {
      mockMvc.perform(MockMvcRequestBuilders.delete("/users/100"))
          ...
          .andExpect(MockMvcResultMatchers.jsonPath("$.error").value("Not Found"))
          .andExpect(
              MockMvcResultMatchers.jsonPath("$.message").value("user not found with id: " + 100))
